...
namespace ControllerService.Modules
{
	...
	public class PmacProgramLoader : IProgramLoader
	{
		...
		private const int CountOfLinesToLoadPrimary = 600;
		private const int CountOfLinesToSecondaryLoad = 300;
		private readonly ManualResetEvent _programStartHandler;
		private static int _loader = 0;
		...
		public PmacProgramLoader(IController controller, IUserSettingsService settings, ILoggerFacade logger, IControllerConfigurator controllerConfigurator, ICodeCalculator codeCalculator, IEventAggregator eventAggregator)
		{
			...
			RotaryBufferTaskScheduler = TaskScheduler.Default;
		}

		public TaskScheduler RotaryBufferTaskScheduler { get; }

		#region auto

		public async Task<IEnumerable<string>> OpenProgramFileNextAsync(string programPath,
			CancellationToken cancellationToken)
		{
			...
			var pieceOfProgram = await Task.Factory.StartNew(() =>
			{
			...
			await PrepareProgramAsync(); //Загрузка программы в ротари буфер

			return pieceOfProgram;
		}
		[Obsolete("You should use OpenProgramFileNextAsync")]
		...
		public bool StartProgram(int line = 0)
		{
			...
			if (result)
			{
				...
				Task.Factory.StartNew(DoCheckProgramStringNumber, _rotaryBufferCancellationToken.Token);
			}
		...
		public async Task InitNewRotaryBuffer(bool mdi = false)
		{
			...
			await Task.Run(() =>
			{
				...
				_controller.GetResponse("&1 DEFINE LOOKAHEAD 1000,100", out _);
		...
		private void DoCheckProgramStringNumber(object t)
		{
			CancellationToken token = (CancellationToken)t;
			var child = Task.Factory.StartNew(DoRotLoad, token);
			child.ContinueWith(WhenRotaryBufferLoadingIsFall, TaskContinuationOptions.OnlyOnFaulted);
			child.ContinueWith((obj) => PrepareProgramAsync().Wait(), TaskContinuationOptions.OnlyOnRanToCompletion);
			while (!child.IsCompleted && !token.IsCancellationRequested)
			{
		...
		private void DoRotLoad(object t)
		{
			CancellationToken token = (CancellationToken) t;
			...
			PrimaryLoadNStrings(ref loadedStringNumber, ref currentToolNumber, loadedStringCount: loadedStringNumber + CountOfLinesToLoadPrimary);
			_programStartHandler.Set(); //Снимает статус блокировки кнопок запуска. Сообщает, что программа готова к запуску.

			_eventAggregator.GetEvent<ProgramDoneToRunEvent>().Publish();

			while (loadedStringNumber < _programFile.StringCount && !token.IsCancellationRequested)
			{
			...
			while ((currentStringNumber < loadedStringNumber || coordinateSystemInPosition == 0) && isProgramRunning == 1 && !token.IsCancellationRequested); //Ожидание окончания программы
		...
		{
			Debug.WriteLine("All were cancelled");
		}
		...
		{
			while ((loadedStringNumber - currentStringNumber) < CountOfLinesToSecondaryLoad)
			{
				_controllerConfigurator.GetVariable(MVariables.ProgramLineNumberFirst, out currentStringNumber);
				if (loadedStringNumber == _programFile.StringCount) //Программа закончилась
					break;
		...
		private void WhenRotaryBufferLoadingIsFall(Task task)
		{
			...
			if (task.Exception != null)
			{
				...
				{
					_logger.Log($"Exception: {item.GetType().Name} Message: {item.Message}. Stack: {item.StackTrace}",
		...
		{
			get => _isProgramRun;
			private set => SetProperty(ref _isProgramRun, value);
		}
		...
		public int ProgramLinesCount => _programFile?.StringCount ?? 0;
