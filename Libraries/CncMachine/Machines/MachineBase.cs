...
namespace CncMachine.Machines
{
    public class MachineBase
    {
        ...
        public virtual async Task LoadProgramAsync(string path)
        {
            Program.Clear();
            GCodePreparer preparer = new GCodePreparer();
            await preparer.OpenFileAsync(path).ConfigureAwait(false);
            await preparer.PrepareStringsAsync().ConfigureAwait(false);
        ...
        private void InterpretProgram(GCodePreparer preparer)
        {
            ...
            foreach (var line in preparer.Strings)
            {
                Context context = new Context();
                context.PreviousFrame = Program.LastOrDefault();
